...
public class PartnerServiceImpl implements PartnerService {
    ...
    public PartnerDto findPartnerById(Long userId) {
        return this.partnerRepository.findById(userId)
                .map(p -> new PartnerDto(p.getId(), p.getPartnerName(), p.getType()))
                .orElseThrow(() -> new PartnerNotFoundException(CommonUtil.NO_RESOURCE_FOUND_WITH_ID));
    }

    @Override
    public List<PartnerDto> getAllPartners() {
        return this.partnerRepository.findAll().stream().map(p -> new PartnerDto(p.getId(), p.getPartnerName(), p.getType())).toList();
    }
